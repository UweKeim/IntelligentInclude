namespace IntelligentInclude
{
	...
	public sealed class Program
	{
		private static int Main(string[] args)
		{
			try
			{
				...
				if (checkPrintArgs(args))
				{
					...
					var pathInformation = CreatePathInformation(rawPath);
					if (pathInformation != null)
					{
						Process(pathInformation, args.Length > 1 && isRecursive(args[1]));
						return 0;
					}
					else
					{
		...
		public static PathInformation CreatePathInformation(string rawPath)
		{
			if (string.IsNullOrEmpty(rawPath))
			{
				Console.Error.WriteLine("[ERROR] Empty path parameter specified.");
				return null;
			}
			else
			{
				...
				{
					Console.WriteLine("Detected '{0}' as folder path, processing all files (*.*).", rawPath);
				...
				{
					Console.Error.WriteLine("[ERROR] Specified directory '{0}' does not exist.", rawPath);
					return null;
				}
				else
				{
					...
					{
						Console.WriteLine("Detected '{0}' as folder path, processing file(s) '{1}'.", folder, fn);
						return new PathInformation { Filename = fn, Folder = folder };
					}
					else if (string.IsNullOrEmpty(folder))
					{
						Console.Error.WriteLine("[ERROR] Folder for '{0}' cannot be detected (is NULL).", rawPath);
						return null;
					}
					else
					{
						Console.Error.WriteLine("[ERROR] Detected '{0}' as folder path, but does not exist.", folder);
						return null;
					}
				}
			}
		...
		public static void Process(PathInformation pathInformation, bool recurse)
		{
			...
			Console.WriteLine("Processing {0} file(s)...", files.Length);

			var index = 0;
			foreach (var file in files)
			{
				Console.WriteLine("Processing file {0} of {1}: '{2}'.", ++index, files.Length, file);
				doProcessFile(file);
			}

			Console.WriteLine("Finished processing {0} file(s).", files.Length);
		}

		private static void doProcessFile(string filePath)
		{
			try
			{
				new FileProcessor(filePath).Process();
			}
			catch (Exception x)
			{
				Console.Error.WriteLine("[ERROR] Error while processing file '{0}': {1}", filePath, x.Message);
				throw;
			}
		}
